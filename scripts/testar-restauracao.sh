...
if [[ -z "$ARQUIVO" ]]; then
  ...
  [[ -n "$ARQUIVO" ]] || erro "nenhuma cópia encontrada em $BACKUP_DIR"
fi
...
printf 'Subindo PostgreSQL descartável...\n'
...
[[ "$pronto" -eq 1 ]] || erro 'PostgreSQL temporário não ficou pronto'

printf 'Restaurando %s...\n' "$(basename "$ARQUIVO")"
if ! docker exec -i "$CONTEINER" pg_restore \
  --username=lucien --dbname=restauracao --no-owner --no-privileges \
  < "$DUMP"; then
  erro 'pg_restore falhou; esta cópia NÃO serve para recuperação'
fi
...
if [[ "${ADMINS:-0}" -ge 1 ]]; then
  printf '  OK      %-14s %s ativo(s)\n' "admin" "$ADMINS"
else
  printf '  FALHOU  %-14s nenhum admin ativo na cópia\n' "admin"
  FALHOU=1
fi
...
if [[ "$FALHOU" -ne 0 ]]; then
  printf '\nA restauração completou mas o conteúdo não confere.\n'
  exit 1
fi

printf '\nRestauração verificada com sucesso.\n'
printf 'Registre a evidência em docs/operacao.md: data, arquivo e resultado.\n'
