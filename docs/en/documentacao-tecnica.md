...
| `RUNBOOK_DOMAIN_FUNCTIONS` | Hub and wiki-builder | accepted domain functions, comma-separated; governs `lucien start -r`, user creation, and jump server enrollment. In the builder, it lists in the index the areas that still have no runbook. Default `platform,access,servers,networks,support`. `platform` is the first administrator's area: if you declare your own list without it, pass `domain_function` when creating the administrator — the Hub refuses an undeclared area |
