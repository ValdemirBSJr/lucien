...
| `lucien admin user issue-provisional-token <user-id-ou-name>` (alias: `rotate-token`) | `user-id-ou-name` (obrigatório) | — | Invalida o token permanente do usuário e emite um novo token provisório de 4 horas. |
| `lucien admin user update <user-id-ou-name>` | `user-id-ou-name` (obrigatório) | `--level <string>` — novo nível de permissão.<br>`-r, --role <string>` — áreas separadas por vírgula, **substituindo** o conjunto atual; a primeira é a primária. | Atualiza nível e/ou áreas de um usuário. Pelo menos uma das duas flags é obrigatória. Omitir `-r` preserva as áreas atuais; informá-la substitui o conjunto inteiro. |
| `lucien admin user revoke <user-id-ou-name>` | `user-id-ou-name` (obrigatório) | `--yes` (bool, padrão `false`, obrigatório) — confirma a revogação do usuário. | Revoga o token do usuário imediatamente. Sem `--yes` o comando recusa executar. |
| `lucien admin user reinstate <user-id-ou-name>` | `user-id-ou-name` (obrigatório) | `--yes` (bool, padrão `false`, obrigatório) — confirma a readmissão do usuário. | Revoga o token do usuário imediatamente. Sem `--yes` o comando recusa executar. |
