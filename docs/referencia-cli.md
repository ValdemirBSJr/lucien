...
| `lucien runbook list` | — | — | Lista os runbooks publicados que o usuário alcança pela área — todas as que ele tem, primária e adicionais; o admin vê todas. Mostra NAME, ID, AREA e PUBLISHED AT, só com a **versão atual** de cada runbook: uma revisão cria versão nova com outro UUID, e o `revise` recusa a anterior com conflito. Somente consulta, não altera nada. A coluna ID é o que `runbook cat` e `runbook revise` recebem. Estar na lista não dispensa o nível: o `revise` continua exigindo `senior` ou `admin` (ou `RBAC_ENTRY_ROLES_ENABLED=true`). A tabela vai para stdout e a dica de uso para stderr, então dá para filtrar com pipe. |
